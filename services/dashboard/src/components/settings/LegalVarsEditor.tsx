...
interface LegalVarsEditorProps {
  value: Record<string, string>;
  onChange: (value: Record<string, string>) => void;
  requiredKeys?: string[];
}

export function LegalVarsEditor({
  value,
  onChange,
  requiredKeys = [],
}: LegalVarsEditorProps): React.ReactElement {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-[var(--text-muted)]">
        {"These variables fill {{placeholders}} in legal page templates."}
      </p>

      {entries.length === 0 && (
        <p className="text-xs text-[var(--text-muted)]">No legal template variables defined.</p>
      )}
          ...
          {entries.map(([key, val]) => (
            ...
            >
              <div className="flex items-center gap-1">
                {requiredSet.has(key) && (
                  <span className="text-red-400 text-sm" title="Required by legal templates">
                    *
                  </span>
                )}
                <input
      ...
      {requiredKeys.length > 0 && (
        <p className="text-xs text-[var(--text-muted)]">
          <span className="text-red-400">*</span> Required by legal templates
        </p>
      )}
    </div>
  );
