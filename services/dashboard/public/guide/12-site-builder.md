...
detect job — for each site in sites/*/, decide "rebuild?"
        ...
        │     • overrides/config/<id>.yaml changed → rebuild sites targeted by that override
        │
        ▼
deploy job (matrix, one per site)
        │
        ├── resolve-config.ts
        │     merges org → groups (left-to-right) → overrides (by priority) → site
...
Build time is where everything that must be right from first paint gets baked in. The Astro layer produces **no** ad divs, **no** ad network scripts, and **no** runtime ad logic. What it does produce:
...
```html
<script>
  window.__ATL_CONFIG__ = {"domain":"coolnews-atl","groups":["entertainment","taboola"], ... };
</script>
...
`test-ads` is now an override (`overrides/config/test-ads-mock.yaml`) that targets specific sites. It uses `provider: mock`, which means `ad-loader.js` hands off to `mock-ad-fill.js`. That script paints a visible labeled box at every placement ("320x50 mobile anchor", "336x280 in-content #1", ...) so you can verify placement logic end-to-end without a real ad network. Remove the override targeting (or switch the site's groups to a real ad partner group) and the same HTML structurally just works — only the runtime provider swaps.
...
| Edit `overrides/config/<id>.yaml` | All sites targeted by that override |
...
Override and group configs live on `main`. Staging builds read whatever snapshot exists on the staging branch, so live rebuilds always reflect the latest dashboard edit while staging is insulated until you explicitly sync.
...
| "Are my ads actually inline?" | View source on the built page — search for `__ATL_CONFIG__` |
| "Is the runtime loader picking them up?" | Browser DevTools → Console — `ad-loader.js` logs which tier supplied the config |
| "Is the site still on the old config?" | Check the build SHA shown in the Pages deployment — compare with the latest config commit |

See **Config Inheritance & Groups** for the upstream merge rules and **Overrides & Config** for the dashboard-level edit path.
