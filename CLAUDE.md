...
22. **Site deletion is two-phase.** Soft delete (`deleteSiteEntry`) moves to trash: disconnects custom domain (deregisters CF, deletes prod KV hostname, reverts config.domain), deletes Git main files, removes prod KV hostname — but **preserves staging branch and R2 images** so the site can be restored. Permanent delete (`permanentlyDeleteSite`) destroys staging branch, all KV entries (both namespaces, including `cond-overrides`), all R2 assets, and records a `history[]` entry in `dashboard-index.yaml`. Restore from trash works because staging + R2 are intact — restored sites get status "Staging". After permanent delete, the site only exists in the `history[]` audit trail. R2 cleanup requires `R2_ACCESS_KEY_ID` + `R2_SECRET_ACCESS_KEY` env vars — logs a warning and skips if not configured.
